...
Transmission Risk Group | Number with PrEP indications | %
------------------------ | ------:| -----:
...
Race by Transmission Risk Group | Number with PrEP indications | % (within transmission risk group)
------------------------ | ------:| -----:
...
ggplot(data = msmnums2, aes(x = Group, y = Individuals, fill = Group)) +
  geom_bar(stat = "identity") +
  geom_text(aes(label = Individuals), vjust = 1.6, color = "white", size = 3.5) +
  theme_minimal() + labs(title = "MSM Indicated for PrEP by Transmission Risk Group", 
